...
( function ( $ ) {
	...
	window.WCEFPModals = {
		 ...
		 * @param {boolean} dismissible - Whether notice can be dismissed
		 */
		showNotice( message, type = 'info', dismissible = true ) {
			const noticeClass =
				`notice notice-${ type }` +
		...
		showConfirm( message, onConfirm, onCancel = null, options = {} ) {
                        ...
                        <div class="wcefp-modal-content">
                            <div class="wcefp-modal-header">
                                <h2 id="${ modalId }-title" class="wcefp-modal-title">
                                    <span class="dashicons dashicons-${ this.getIconByType(
										opts.type
									) }"></span>
                                    ${ opts.title }
                                </h2>
                            ...
                            <div class="wcefp-modal-footer">
                                <button type="button" class="button button-primary wcefp-modal-confirm">${
									opts.confirmText
								}</button>
                                <button type="button" class="button wcefp-modal-cancel">${
									opts.cancelText
								}</button>
                            </div>
                        </div>
			...
			// Handle cancel/close
			modal
				.find( '.wcefp-modal-cancel, .wcefp-modal-close' )
				.on( 'click', function () {
