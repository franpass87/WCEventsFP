...
document.addEventListener('DOMContentLoaded', function() {
    const WCEFPGoogleReviews = {
        ...
        async loadReviews() {
            const reviewsContainer = document.querySelector('.wcefp-google-reviews');
            ...
            const html = `
                <div class="google-reviews-header">
                    ...
                    <div class="overall-rating">
                        <div class="stars">${this.renderStars(overall_rating)}</div>
                        <span class="rating-text">${overall_rating}/5 su Google</span>
                    </div>
                </div>
                <div class="reviews-list">
                    ${reviews.map(review => this.renderSingleReview(review)).join('')}
                </div>
                    ...
                    <div class="review-header">
                        <div class="author-info">
                            ${review.profile_photo_url ? 
                                ...
                                `<div class="author-avatar-placeholder">${review.author_name.charAt(0)}</div>`
                            }
                            <div class="author-details">
                                <span class="author-name">${review.author_name}</span>
                                <span class="review-date">${review.relative_time_description || timeago}</span>
                            </div>
                        </div>
        ...
        getTimeAgo(timestamp) {
            ...
            for (const interval of intervals) {
                ...
                if (count > 0) {
                    return count === 1 ? `1 ${interval.label} fa` : `${count} ${interval.label}${count > 1 && interval.label !== 'mese' ? 'i' : ''} fa`;
                }
            }
