...
(function($) {
    ...
    class WCEFPInternationalization {
        ...
        bindEvents() {
            ...
            $(document).on('click', '.wcefp-language-switcher .wcefp-locale-option', (e) => {
                e.preventDefault();
        ...
        initLanguageSwitcher() {
            ...
            const currentLocaleInfo = this.supportedLocales[this.currentLocale];
            ...
            const switcher = $(`
                <div class="wcefp-language-switcher">
                    <button class="wcefp-current-language" aria-label="Change Language">
                        <span class="wcefp-flag">${currentLocaleInfo.flag}</span>
                        <span class="wcefp-language-name">${currentLocaleInfo.name}</span>
        ...
        generateLanguageOptions() {
            ...
            const sortedLocales = Object.entries(this.supportedLocales)
        ...
        async switchLocale(locale) {
            ...
            try {
                ...
                if (response.success) {
                    ...
                    $(document).trigger('wcefp_locale_changed', [locale, this.supportedLocales[locale]]);
        ...
        extractStringsFromPage() {
            ...
            const commonStrings = [
                'Book Now', 'Check Availability', 'Select Date', 'Participants',
                'Total Price', 'Confirm Booking', 'Duration', 'Meeting Point',
                'What\'s Included', 'Reviews', 'Loading...', 'Cancel Booking'
            ];
        ...
        translateBookingElements() {
            ...
            Object.entries(selectors).forEach(([selector, translationKey]) => {
                $(selector).each((index, element) => {
                    const $element = $(element);
                    const translation = this.getTranslation(translationKey, 'booking');
                    
                    if (translation && !$element.data('original-text')) {
        ...
        getTranslation(key, category = null) {
            if (category && this.translations[category] && this.translations[category][key]) {
        ...
        updateLanguageSwitcher(newLocale) {
            ...
            $switcher.find('.wcefp-language-dropdown').html(this.generateLanguageOptions());
        ...
        localizeBookingForm() {
            ...
            $bookingForm.find('label[for="wcefp-date"]').text(this.getTranslation('select_date', 'booking'));
            $bookingForm.find('label[for="wcefp-participants"]').text(this.getTranslation('participants', 'booking'));
            $bookingForm.find('.wcefp-total-price').text(this.getTranslation('total_price', 'booking'));
            
            // Update button texts
            $bookingForm.find('.wcefp-book-button').text(this.getTranslation('book_now', 'booking'));
            $bookingForm.find('.wcefp-availability-button').text(this.getTranslation('check_availability', 'booking'));
        }
        ...
        updateBookingFormLocalization() {
            ...
            $(document).trigger('wcefp_booking_form_localized', [this.currentLocale]);
        }
                ...
                ">
                    ...
                    ">
                        <div style="
                            ...
                            margin: 0 auto 10px;
                        "></div>
                        <div>${this.getTranslation('loading', 'booking')}</div>
                    </div>
                </div>
            `);
         ...
         */
        loadSavedLocale() {
            const savedLocale = localStorage.getItem('wcefp_preferred_locale');
            if (savedLocale && this.supportedLocales[savedLocale] && savedLocale !== this.currentLocale) {
        ...
        if (typeof wcefp_i18n !== 'undefined') {
            ...
            if (typeof WCEFPAnalytics !== 'undefined' && window.wcefpAnalytics) {
                window.wcefpAnalytics.track('i18n_initialized', {
                    ...
                    supported_locales: Object.keys(window.wcefpI18n.supportedLocales)
                });
            }
        }
    });
