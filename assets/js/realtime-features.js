	...
	class WCEFPRealtimeClient {
		...
		async connect() {
			...
			try {
				...
				if ( response.success ) {
					...
					console.log(
						'WCEventsFP: Real-time connection established'
					);
				} else {
					throw new Error(
		...
		async pollUpdates() {
			...
			try {
				...
				if ( response.success ) {
					...
					response.data?.msg?.includes( 'Session expired' )
				) {
		...
		handleBookingUpdate( update ) {
			...
			if ( window.WCEFPNotifications ) {
				window.WCEFPNotifications.show( update.message, 'info', 5000 );
			}
		...
		updateAvailabilityIndicators( update ) {
			...
			selectors.forEach( ( selector ) => {
				...
				elements.each( ( index, element ) => {
					...
					const availabilityEl = $el.find(
						'.wcefp-availability, .availability'
					);
					if ( availabilityEl.length ) {
						...
						availabilityEl
							.removeClass(
								'available critical limited sold-out'
							)
							.addClass( className )
							.html( text );
					}
					...
					if ( progressEl.length && update.capacity > 0 ) {
						const percentage =
							( update.booked / update.capacity ) * 100;
						progressEl
							.find( '.progress-fill' )
		...
		updateBookingButtons( update ) {
			const buttons = $(
				`[data-occurrence-id="${ update.occurrence_id }"] .wcefp-book-btn, [data-product-id="${ update.product_id }"] .wcefp-book-btn`
			);
		...
		handleConnectionError( error ) {
			...
			if ( this.reconnectAttempts < this.maxReconnectAttempts ) {
				...
				setTimeout( () => {
					console.log(
						`WCEventsFP: Reconnecting... (attempt ${ this.reconnectAttempts })`
					);
					this.connect();
				}, delay );
			} else {
				console.error(
					'WCEventsFP: Max reconnection attempts reached'
				);
		...
		if ( typeof WCEFPRealtime !== 'undefined' ) {
			...
			if ( $( '#wpadminbar' ).length ) {
				const statusIndicator = $(
					'<div id="wcefp-realtime-status" style="position: fixed; top: 32px; right: 20px; z-index: 99999; padding: 5px 10px; background: #f0f0f0; border-radius: 3px; font-size: 11px; display: none;">Real-time: <span class="status">Connecting...</span></div>'
				);
