	...
	window.WCEFPVoucherManager = {
                    ...
                    <div style="display: flex; gap: 15px; align-items: center; flex-wrap: wrap;">
                        <div>
                            <label for="voucher-search" style="margin-right: 8px;">
                                ${
									wcefpVoucherManager.strings.search ||
									'Cerca'
								}:
                            </label>
                            <input type="text" 
                                   id="voucher-search" 
                                   placeholder="${
										wcefpVoucherManager.strings
											.search_placeholder ||
										'Codice, email, destinatario...'
									}"
                                   style="width: 200px;">
                        </div>
                        <div>
                            ...
                            <select id="voucher-status-filter">
                                <option value="">${
									wcefpVoucherManager.strings.all_statuses ||
									'Tutti gli stati'
								}</option>
                                <option value="active">${
									wcefpVoucherManager.strings.status_active ||
									'Attivo'
								}</option>
                                <option value="redeemed">${
									wcefpVoucherManager.strings
										.status_redeemed || 'Utilizzato'
								}</option>
                        ...
                        <button class="button wcefp-show-analytics">
                            📊 ${
								wcefpVoucherManager.strings.show_analytics ||
								'Mostra Statistiche'
							}
                        </button>
                    </div>
		...
		enhanceTableRows() {
			...
			$( '.wp-list-table tbody tr' ).each( function () {
				...
				const status =
					$row.find( '.voucher-status' ).data( 'status' ) ||
				...
				if ( $actions.length === 0 ) {
					$actions = $(
						'<div class="voucher-quick-actions" style="margin-top: 5px;"></div>'
					);
				...
				buttons.push( `
                    <button class="button button-small wcefp-voucher-details" 
                            ...
                            title="${
								wcefpVoucherManager.strings.view_details ||
								'Visualizza dettagli'
							}">
                        👁️
                    </button>
				...
				if ( status === 'active' ) {
					buttons.push( `
                        <button class="button button-small wcefp-voucher-action" 
                                ...
                                data-voucher-code="${ voucherCode }"
                                title="${
									wcefpVoucherManager.strings.resend_email ||
									'Reinvia email'
								}">
                            📧
                        </button>
					...
					buttons.push( `
                        <button class="button button-small button-link-delete wcefp-voucher-action" 
                                ...
                                title="${
									wcefpVoucherManager.strings
										.cancel_voucher || 'Annulla voucher'
								}">
                            ❌
                        </button>
                    ` );
		 ...
		 * @param  $button
		 */
		handleVoucherAction( action, voucherCode, $button ) {
			...
			switch ( action ) {
				...
				default:
					confirmMessage =
						wcefpVoucherManager.strings.confirm_action ||
						'Confermi questa azione?';
			}

			// Show confirmation modal
			WCEFPModals.showConfirm( confirmMessage, function () {
		 ...
		 * @param  $button
		 */
		performVoucherAction( action, voucherCode, $button ) {
			const self = this;
					...
					if ( response.success ) {
						...
						WCEFPModals.showError(
							response.data.message ||
								wcefpVoucherManager.strings.error_occurred
						);
					}
				},
		...
		showVoucherDetails( voucherCode ) {
			...
			$.ajax( {
				...
				success( response ) {
					if ( response.success ) {
						$content.html(
							self.formatVoucherDetails( response.data )
						);
					} else {
						$content.html( `
                            <div class="error">
		...
		formatVoucherDetails( data ) {
			...
			let html = `
                <div class="voucher-details-grid" style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div class="voucher-basic-info">
                        <h3>${
							wcefpVoucherManager.strings.basic_info ||
							'Informazioni Base'
						}</h3>
                        <table class="widefat">
                            <tbody>
                                ...
                                <tr>
                                    <td><strong>${
										wcefpVoucherManager.strings.amount ||
										'Importo'
									}:</strong></td>
                                ...
                                <tr>
                                    <td><strong>${
										wcefpVoucherManager.strings.status ||
										'Stato'
									}:</strong></td>
                                    <td><span class="voucher-status status-${
										voucher.status
									}">${ data.status_label }</span></td>
                                </tr>
                                <tr>
                                    <td><strong>${
										wcefpVoucherManager.strings.created ||
										'Creato'
									}:</strong></td>
                                    <td>${ this.formatDate(
			...
			if ( voucher.expiry_date ) {
				html += `
                                <tr>
                                    <td><strong>${
										wcefpVoucherManager.strings.expires ||
										'Scadenza'
									}:</strong></td>
                                    <td>${ this.formatDate(
										voucher.expiry_date
			...
			html += `
                    ...
                    <div class="voucher-recipient-info">
                        <h3>${
							wcefpVoucherManager.strings.recipient_info ||
			...
			if ( voucher.message ) {
				html += `
                                <tr>
                                    <td><strong>${
										wcefpVoucherManager.strings.message ||
										'Messaggio'
									}:</strong></td>
			...
			if ( data.usage_history && data.usage_history.length > 0 ) {
				html += `
                    <div class="voucher-usage-history" style="margin-top: 20px;">
                        <h3>${
							wcefpVoucherManager.strings.usage_history ||
							'Cronologia Utilizzo'
						}</h3>
                        <table class="widefat">
                            <thead>
                                <tr>
                                    <th>${
										wcefpVoucherManager.strings.date ||
										'Data'
									}</th>
                                    <th>${
										wcefpVoucherManager.strings.order ||
										'Ordine'
									}</th>
                                    <th>${
										wcefpVoucherManager.strings.customer ||
										'Cliente'
									}</th>
                                    <th>${
										wcefpVoucherManager.strings
			...
			// Add action buttons
			html +=
				'<div class="voucher-details-actions" style="margin-top: 20px; text-align: right;">';
			...
			if ( data.can_cancel ) {
				html += `
                    <button class="button button-link-delete wcefp-voucher-action" 
                            ...
                            style="margin-left: 10px;">
                        ❌ ${
							wcefpVoucherManager.strings.cancel_voucher ||
							'Annulla Voucher'
						}
                    </button>
                `;
		...
		formatAnalytics( data ) {
			let html = `
                ...
                <div class="analytics-details" style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px;">
                    <div class="status-breakdown">
                        <h3>${
							wcefpVoucherManager.strings.status_breakdown ||
                        ...
                        <table class="widefat">
                            <thead>
                                <tr>
                                    ...
                                    <th>${
										wcefpVoucherManager.strings.count ||
										'Numero'
									}</th>
                                    <th>${
										wcefpVoucherManager.strings
			...
			if ( data.status_breakdown && data.status_breakdown.length > 0 ) {
				...
				html += `
                                <tr>
                                    <td colspan="3">${
										wcefpVoucherManager.strings.no_data ||
			...
			html += `
                    ...
                    <div class="monthly-stats">
                        <h3>${
							wcefpVoucherManager.strings.monthly_stats ||
							'Statistiche Mensili'
						}</h3>
                        <table class="widefat">
                            <thead>
                                <tr>
                                    <th>${
										wcefpVoucherManager.strings.month ||
										'Mese'
									}</th>
                                    <th>${
										wcefpVoucherManager.strings.created ||
                                    ...
                                    <th>${
										wcefpVoucherManager.strings.value ||
										'Valore'
									}</th>
                                </tr>
                            </thead>
		 ...
		 * @param  newStatus
		 */
		updateVoucherRowStatus( voucherCode, newStatus ) {
			$( '.wp-list-table tbody tr' ).each( function () {
				const $row = $( this );
				const rowVoucherCode = $row
					.find( '.voucher-code' )
					.text()
					.trim();
				...
				if ( rowVoucherCode === voucherCode ) {
					...
					$statusCell
						.removeClass(
							'status-active status-redeemed status-expired status-cancelled'
						)
		...
		getStatusLabel( status ) {
			const labels = {
				...
				redeemed:
					wcefpVoucherManager.strings.status_redeemed || 'Utilizzato',
				expired:
					wcefpVoucherManager.strings.status_expired || 'Scaduto',
				cancelled:
					wcefpVoucherManager.strings.status_cancelled || 'Annullato',
			};
