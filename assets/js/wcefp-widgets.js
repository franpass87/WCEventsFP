    ...
    window.WCEFPWidgets = {
        ...
        initFormWidgets: function() {
            ...
            $('.wcefp-widget .wcefp-form-control').on('input', function() {
                const $field = $(this);
                if ($field.closest('.wcefp-form-group').hasClass('has-error')) {
        ...
        initBookingWidgets: function() {
            ...
            $('.wcefp-widget input[type="date"]').each(function() {
                ...
                if (!$input.next('.wcefp-form-help').length) {
                    $input.after('<span class="wcefp-form-help">Format: DD/MM/YYYY</span>');
                }
            });
        ...
        initValidation: function() {
            ...
            $('.wcefp-widget form').on('submit', function(e) {
                ...
                $form.find('.wcefp-form-control[required]').each(function() {
                ...
                if (!isValid) {
                    e.preventDefault();
                    self.showMessage($form, 'error', 'Please correct the errors below.');
                    
                    // Focus on first error field
                    const $firstError = $form.find('.has-error .wcefp-form-control').first();
        ...
        initAccessibility: function() {
            ...
            $('.wcefp-widget .wcefp-form-control').each(function() {
                const $field = $(this);
                const $label = $field.closest('.wcefp-form-group').find('.wcefp-form-label');
                const $help = $field.siblings('.wcefp-form-help');
                
                // Connect label to field
                if ($label.length && !$field.attr('aria-labelledby')) {
                    const labelId = 'wcefp-label-' + Math.random().toString(36).substr(2, 9);
                ...
                if ($help.length && !$field.attr('aria-describedby')) {
                    const helpId = 'wcefp-help-' + Math.random().toString(36).substr(2, 9);
                    $help.attr('id', helpId);
        ...
        initResponsive: function() {
            ...
            const handleResize = () => {
                $('.wcefp-widget').each(function() {
                    ...
                    $widget.toggleClass('wcefp-tablet', width >= 600 && width < 900);
        ...
        validateField: function($field) {
            ...
            if (isValid) {
                ...
                $field.after(`<span class="wcefp-form-help error" role="alert">${errorMessage}</span>`);
            }
        ...
        enhanceSelect: function($select) {
            ...
            $select.on('change', function() {
                const $dependentSelects = $('.wcefp-widget select[data-depends="' + $select.attr('id') + '"]');
         ...
         */
        setLoading: function($element, show) {
            if (show) {
                $element.addClass('wcefp-loading').prop('disabled', true);
            } else {
                $element.removeClass('wcefp-loading').prop('disabled', false);
            }
        },
        ...
        showMessage: function($container, type, message) {
            ...
            if (messageTop < windowTop || messageTop > windowTop + windowHeight) {
                $('html, body').animate({
        ...
        submitForm: function($form, action, callback) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        self.showMessage($form, 'success', response.data.message || 'Form submitted successfully!');
                        if (typeof callback === 'function') {
                            callback(response);
                        }
                        $form[0].reset(); // Reset form on success
                    } else {
                        self.showMessage($form, 'error', response.data.message || 'An error occurred. Please try again.');
                    }
                },
                error: function() {
                    self.showMessage($form, 'error', 'Connection error. Please check your internet connection and try again.');
                },
                complete: function() {
         ...
         */
        loadAsset: function(assetType, url, callback) {
            if (assetType === 'css') {
