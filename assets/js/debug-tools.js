...
( function ( $ ) {
	...
	class WCEFPDebugTools {
		...
		switchTab( tab ) {
			...
			// Track tab switch
			this.trackEvent( 'debug_tab_switched', { tab } );
		}
		...
		loadPerformanceData() {
			...
			$container.html(
				'<div class="wcefp-loading">Loading performance data...</div>'
			);
			...
			$.post( wcefp_debug.ajax_url, {
				...
				.done( ( response ) => {
					if ( response.success ) {
						...
						$container.html(
							'<div class="wcefp-error">Failed to load performance data</div>'
						);
					}
				} )
				.fail( () => {
					$container.html(
						'<div class="wcefp-error">Network error loading performance data</div>'
					);
				} );
		}
		...
		renderPerformanceData( data ) {
			...
			html += `<div class="wcefp-metric">Memory Usage: ${ this.formatBytes(
				data.memory_usage
			) }</div>`;
			html += `<div class="wcefp-metric">Peak Memory: ${ this.formatBytes(
				data.memory_peak
			) }</div>`;
			...
			// Performance markers
			if (
				data.performance_markers &&
				Object.keys( data.performance_markers ).length > 0
			) {
				html += '<h4>Performance Markers</h4>';
				html += '<div class="wcefp-markers">';

				Object.entries( data.performance_markers ).forEach(
					( [ name, marker ] ) => {
						if ( name === 'calculated_metrics' ) {
							html += '<h5>Calculated Metrics</h5>';
							html += `<div class="wcefp-metric">Total Time: ${ marker.total_time?.toFixed(
								2
							) }ms</div>`;
							html += `<div class="wcefp-metric">Memory Used: ${ this.formatBytes(
								marker.memory_used
							) }</div>`;
							...
							html += `<br><small>Memory: ${ this.formatBytes(
								marker.memory
							) }</small>`;
							html += `</div>`;
						}
		...
		loadSystemInfo() {
			...
			$container.html(
				'<div class="wcefp-loading">Loading system information...</div>'
			);
			...
			$.post( wcefp_debug.ajax_url, {
				...
				.done( ( response ) => {
					if ( response.success ) {
						...
						$container.html(
							'<div class="wcefp-error">Failed to load system information</div>'
						);
					}
				} )
				.fail( () => {
					$container.html(
						'<div class="wcefp-error">Network error loading system information</div>'
					);
				} );
		}
		...
		loadQueryInfo() {
			...
			$container.html(
				'<div class="wcefp-loading">Loading query information...</div>'
			);
			...
			$.post( wcefp_debug.ajax_url, {
				...
				.done( ( response ) => {
					if ( response.success ) {
						...
						$container.html(
							'<div class="wcefp-error">Failed to load query information</div>'
						);
					}
				} )
				.fail( () => {
					$container.html(
						'<div class="wcefp-error">Network error loading query information</div>'
					);
				} );
		}
		...
		renderQueryInfo( data ) {
			...
			if ( data.query_log && data.query_log.length > 0 ) {
				...
				data.query_log.forEach( ( entry, index ) => {
					...
					html += `<span class="wcefp-query-number">#${
						index + 1
					}</span>`;
					html += `<span class="wcefp-query-time">${ time }</span>`;
					html += `</div>`;
					html += `<div class="wcefp-query-sql"><code>${ this.escapeHtml(
						entry.query
					) }</code></div>`;
					html += `</div>`;
				} );
				...
				html +=
					'<div class="wcefp-no-queries">No queries logged yet.</div>';
			}
		...
		initLogFiltering() {
			...
			// Bind filter events
			$( '#wcefp-log-level-filter, #wcefp-log-search' ).on(
				'input',
				() => {
		...
		filterLogEntries() {
			...
			$( '.wcefp-debug-entry' ).each( function () {
				const $entry = $( this );
				const level = $entry
					.find( '.wcefp-debug-level' )
					.text()
					.replace( /[\[\]]/g, '' )
					.toLowerCase();
				const message = $entry
					.find( '.wcefp-debug-message' )
					.text()
					.toLowerCase();
		...
		clearLog() {
			if (
				! confirm( 'Are you sure you want to clear the debug log?' )
			) {
				return;
			}
			...
			$.post( wcefp_debug.ajax_url, {
				...
				.done( ( response ) => {
					if ( response.success ) {
						$( '.wcefp-debug-log-entries' ).html(
							'<div class="wcefp-no-entries">Debug log cleared</div>'
						);
				...
				.fail( () => {
					this.showNotification(
						'Network error clearing debug log',
						'error'
					);
				} );
		}
			...
			try {
				...
				const navigationObserver = new PerformanceObserver(
					( list ) => {
						...
						entries.forEach( ( entry ) => {
							this.addClientLog(
								`Page navigation completed in ${ entry.duration.toFixed(
									2
								) }ms`,
								'performance'
							);
		...
		addClientLog( message, level = 'info' ) {
			...
			const logHtml = `
                <div class="wcefp-debug-entry wcefp-debug-${ level } wcefp-client-log">
                    ...
                    <span class="wcefp-debug-message">${ this.escapeHtml(
						message
					) }</span>
	...
	$( document ).ready( function () {
		if ( typeof wcefp_debug !== 'undefined' ) {
			...
			if ( wcefp_debug.is_debug_enabled ) {
				console.log(
					'%cWCEFP Debug Tools: Press Alt+D to toggle debug panel',
