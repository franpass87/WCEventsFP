...
( function ( $ ) {
	...
	const WCEFPAjax = {
		request( action, data = {}, options = {} ) {
			...
			return $.post(
				window.WCEFPAdmin?.ajaxUrl ||
					window.WCEFPClose?.ajaxUrl ||
					ajaxurl,
				ajaxData
			)
				...
				.fail( function ( xhr, status, error ) {
					...
					console.error( 'WCEFP Ajax Error:', {
						xhr,
						status,
						error,
					} );
	...
	const WCEFPValidation = {
		...
		highlightField( $field, isValid = true ) {
			$field
				.toggleClass( 'wcefp-field-error', ! isValid )
	...
	$( document ).ready( function () {
		...
		$( document ).on(
			'input change',
			'.wcefp-field-error, .wcefp-field-valid',
			function () {
