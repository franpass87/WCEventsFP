...
class WCEFPAdvancedAnalytics {
    ...
    createDashboard() {
        const existingDashboard = document.querySelector('.wcefp-advanced-analytics');
        ...
        if (insertTarget) {
            insertTarget.parentNode.insertBefore(dashboard, kpiDashboard ? kpiDashboard.nextSibling : insertTarget.firstChild);
        }
    }
    ...
    updateRealTimeMetrics(realtime) {
        const updates = {
            ...
            'realtime-revenue': `€${this.formatNumber(realtime.revenue_today || 0)}`,
            'realtime-conversion': `${(realtime.conversion_rate || 0).toFixed(1)}%`
        };
    ...
    updateTrendIndicators(trends) {
        ...
        Object.entries(trendElements).forEach(([id, trend]) => {
            ...
            if (element) {
                ...
                const color = isPositive ? '#10b981' : trend < 0 ? '#ef4444' : '#64748b';
    ...
    initializeCharts() {
        // Revenue prediction chart
        const revenueCtx = document.getElementById('revenue-prediction-chart');
        if (revenueCtx) {
        ...
        if (journeyCtx) {
            this.charts.customerJourney = new Chart(journeyCtx, {
                type: 'bar',
                data: {
                    labels: ['Visitatori', 'Interessati', 'Carrello', 'Checkout', 'Acquisto'],
                    datasets: [{
                        label: 'Utenti',
                        data: [],
                        backgroundColor: [
    ...
    updateCharts(data) {
        ...
        if (this.charts.revenuePrediction && data.revenue_data) {
            this.charts.revenuePrediction.data.labels = data.revenue_data.labels;
            this.charts.revenuePrediction.data.datasets[0].data = data.revenue_data.historical;
            this.charts.revenuePrediction.data.datasets[1].data = data.revenue_data.prediction;
            this.charts.revenuePrediction.update();
        }

        // Update customer journey
        if (this.charts.customerJourney && data.journey_data) {
            this.charts.customerJourney.data.datasets[0].data = data.journey_data.values;
            this.charts.customerJourney.update();
        }

        // Update customer segments
        if (this.charts.customerSegments && data.segments_data) {
            this.charts.customerSegments.data.labels = data.segments_data.labels;
            this.charts.customerSegments.data.datasets[0].data = data.segments_data.values;
    ...
    updateBookingHeatmap(patterns) {
        ...
        days.forEach((day, dayIndex) => {
            ...
            hours.forEach(hour => {
                ...
                const intensity = Math.min(value / patterns.max * 100, 100);
    ...
    generatePredictions(data) {
        const model = document.getElementById('prediction-model')?.value || 'linear';
        
        // Simple linear regression for demo
        if (data.revenue_data?.historical) {
            const historical = data.revenue_data.historical;
            const predictions = this.predictWithLinearRegression(historical, 7);
            
            // Update prediction data in chart
            if (this.charts.revenuePrediction) {
                const currentLabels = [...this.charts.revenuePrediction.data.labels];
                const predictionLabels = this.generateFutureDates(currentLabels.length, 7);
                
                this.charts.revenuePrediction.data.labels = [...currentLabels, ...predictionLabels];
                this.charts.revenuePrediction.data.datasets[1].data = [...Array(currentLabels.length).fill(null), ...predictions];
                this.charts.revenuePrediction.update();
            }
        }
    }
    ...
    updateInsights(insights) {
        ...
        const html = insights.map(insight => `
            <div class="wcefp-insight-card ${insight.type}">
                <div class="wcefp-insight-icon">${this.getInsightIcon(insight.type)}</div>
                <div class="wcefp-insight-content">
                    <h4>${insight.title}</h4>
                    <p>${insight.description}</p>
                    ${insight.action ? `<button class="wcefp-insight-action" data-action="${insight.action}">${insight.actionText}</button>` : ''}
                </div>
                <div class="wcefp-insight-impact">${insight.impact}</div>
            </div>
        `).join('');

        container.innerHTML = html || '<p>Nessun insight disponibile al momento</p>';

        // Bind insight action buttons
        container.querySelectorAll('.wcefp-insight-action').forEach(btn => {
    ...
    updateAlerts(alerts) {
            ...
            <div class="wcefp-alerts-list">
                ${alerts.map(alert => `
                    <div class="wcefp-alert ${alert.severity}">
                        <div class="wcefp-alert-icon">${this.getAlertIcon(alert.severity)}</div>
                        <div class="wcefp-alert-content">
                            <strong>${alert.title}</strong>
                            <p>${alert.message}</p>
                            <small>${this.formatDate(alert.timestamp)}</small>
                        </div>
                        <button class="wcefp-alert-dismiss" data-alert-id="${alert.id}">✕</button>
                    </div>
                `).join('')}
            </div>
        `;
        ...
        container.querySelectorAll('.wcefp-alert-dismiss').forEach(btn => {
            btn.addEventListener('click', (e) => {
    ...
    setupRealTimeUpdates() {
        ...
        setInterval(() => {
            const timeRange = document.getElementById('wcefp-time-range')?.value || 30;
    ...
    async exportAnalytics() {
        try {
            ...
            a.href = url;
            a.download = `wcefp-analytics-${new Date().toISOString().split('T')[0]}.csv`;
