...
(function($) {
    ...
    class WCEFPMobileCheckin {
        ...
        bindEvents() {
            // Tab switching
            $('.wcefp-tab-button').on('click', this.handleTabSwitch.bind(this));
            
            // Scanner controls
            $('#wcefp-start-scanner').on('click', this.startScanner.bind(this));
            $('#wcefp-stop-scanner').on('click', this.stopScanner.bind(this));
            
            // Manual check-in form
            $('#wcefp-manual-checkin-form').on('submit', this.handleManualCheckin.bind(this));
            
            // Modal controls
            $('.wcefp-modal-close').on('click', this.closeModal.bind(this));
            $('#wcefp-confirm-checkin').on('click', this.confirmCheckin.bind(this));
            
            // Retry button
            $('#wcefp-retry-checkin').on('click', this.resetInterface.bind(this));
        ...
        async checkCameraPermissions() {
            try {
                ...
                if (permissions.state === 'denied') {
                    this.showScannerMessage(wcefp_checkin.strings.camera_denied || 'Camera access denied. Please use manual entry.', 'error');
        ...
        initScanner() {
            ...
            if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
                this.showScannerMessage(wcefp_checkin.strings.camera_not_supported || 'Camera not supported on this device', 'error');
                $('.wcefp-tab-button[data-tab="manual-entry"]').click();
                return;
            }
        }
        ...
        async startScanner() {
            try {
                ...
                this.showScannerMessage(wcefp_checkin.strings.scan_qr || 'Position QR code within the frame', 'info');
                ...
                this.showScannerMessage(wcefp_checkin.strings.camera_error || 'Failed to access camera', 'error');
                this.showLoading(false);
            }
        }
        ...
        handleQRCode(qrData) {
            ...
            try {
                ...
                if (!token || !bookingId) {
                    this.showError(wcefp_checkin.strings.invalid_qr || 'Invalid QR code');
                    return;
                }
                ...
                this.showError(wcefp_checkin.strings.invalid_qr || 'Invalid QR code format');
            }
        }
        ...
        async getBookingDetails(bookingId, token) {
            try {
                ...
                if (response.success) {
                    ...
                    this.showError(response.data.message || wcefp_checkin.strings.error);
                }
                
            } catch (error) {
                console.error('Error getting booking details:', error);
                this.showError(wcefp_checkin.strings.error || 'An error occurred');
            } finally {
                this.showLoading(false);
            }
        }
        ...
        showBookingModal(bookingData, token) {
            this.currentBookingData = { ...bookingData, token: token };
            
            // Populate modal with booking details
            $('#wcefp-modal-event-title').text(bookingData.event_title || '');
            $('#wcefp-modal-customer-name').text(bookingData.customer_name || '');
            $('#wcefp-modal-booking-date').text(bookingData.booking_date || '');
        ...
        async processCheckin(formData) {
            try {
                ...
                this.showError(wcefp_checkin.strings.error || 'An error occurred during check-in');
            } finally {
                this.showLoading(false);
            }
        }
        ...
        showSuccess(data) {
            const statusText = `${wcefp_checkin.strings.success || 'Check-in successful!'} 
                              ${data.checkin_time ? `at ${data.checkin_time}` : ''}`;
