	...
	class WCEFPConversionOptimizer {
		...
		initSocialProof() {
			$( '.wcefp-card, .wcefp-widget' ).each( function () {
				const $element = $( this );
				const productId =
					$element.data( 'product-id' ) || $element.data( 'product' );
				...
				const $rating = $( `
					...
					5 - Math.floor( parseFloat( rating ) )
				) }
                        </div>
		...
		initDynamicPricingHints() {
			$( '.wcefp-widget .wcefp-price, .wcefp-card .wcefp-price' ).each(
				function () {
					...
					if ( currentPrice > 0 ) {
						...
						if ( Math.random() > 0.6 ) {
							// 40% chance of showing discount
							const originalPrice =
								currentPrice *
								( 1 + Math.random() * 0.3 + 0.1 ); // 10-40% higher
                            ...
                            <div class="wcefp-price-comparison">
                                <span class="wcefp-original-price">€${ originalPrice.toFixed(
									2
								) }</span>
                                <span class="wcefp-savings">Risparmi €${ savings.toFixed(
									2
								) } (${ savingsPercent }%)</span>
                            </div>
                        ` );
		...
		initScarcityIndicators() {
			$( '.wcefp-widget, .wcefp-card' ).each( function () {
				...
				$element
					.find( '.wcefp-availability' )
					.after( $scarcityIndicator );
			} );
		}
		...
		initTrustBadges() {
			...
			$( '.wcefp-widget' ).each( function () {
				...
				const $trustContainer = $(
					'<div class="wcefp-trust-badges"></div>'
				);
		...
		showExitIntentPopup() {
			...
			$popup
				.find( '.wcefp-popup-close, .wcefp-exit-intent-overlay' )
				.on( 'click', ( e ) => {
					...
					const $comparison = $( `
                        <div class="wcefp-price-comparison-table">
                            ...
                            <div class="wcefp-comparison-row wcefp-our-price">
                                ...
                                <span class="wcefp-price-value">€${ currentPrice.toFixed(
									2
								) }</span>
                            ...
                            ${ competitors
                                ...
                                <div class="wcefp-comparison-row">
                                    <span class="wcefp-provider">${
										comp.name
									}</span>
                                    <span class="wcefp-price-value">€${ comp.price.toFixed(
										2
									) }</span>
                                </div>
                            `
	...
	class WCEFPLiveSupport {
		...
		bindEvents() {
			...
			$( document ).on( 'click', '.wcefp-quick-reply', function () {
				...
				const responses = {
					'Info prezzi':
						'I nostri prezzi includono tutto: guida esperta, attrezzatura e assicurazione. Offriamo anche il prezzo garantito più basso!',
					Disponibilità:
						"Controlliamo la disponibilità in tempo reale. Puoi prenotare fino a 2 ore prima dell'esperienza!",
					Cancellazioni:
						"Cancellazione gratuita fino a 24 ore prima dell'esperienza. Rimborso completo garantito!",
				};
				...
				setTimeout( () => {
					this.addMessage(
						responses[ reply ] ||
							'Grazie per la tua domanda. Un operatore ti risponderà presto!',
						'bot'
					);
				}, 1000 );
			} );
			...
			$( document ).on( 'click', '.wcefp-chat-send', () => {
				...
				if ( message ) {
					...
					setTimeout( () => {
						this.addMessage(
							'Grazie per il tuo messaggio! Un operatore ti risponderà il prima possibile.',
							'bot'
						);
					}, 1500 );
				}
		...
		addMessage( text, type ) {
			...
			$( '.wcefp-chat-messages' )[ 0 ].scrollTop = $(
				'.wcefp-chat-messages'
			)[ 0 ].scrollHeight;
		}
	}
