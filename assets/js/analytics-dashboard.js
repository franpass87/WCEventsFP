...
(function($) {
    ...
    class WCEFPAnalyticsDashboard {
        ...
        bindEvents() {
            // Date range filter
            $(document).on('change', '#wcefp-date-range-filter', this.handleDateRangeChange.bind(this));
            
            // Refresh button
            $(document).on('click', '#wcefp-refresh-dashboard', this.refreshDashboard.bind(this));
            
            // Export buttons
            $(document).on('click', '.wcefp-export-btn', this.handleExport.bind(this));
            
            // Chart interactions
            $(document).on('click', '.wcefp-chart-filter', this.handleChartFilter.bind(this));
        }
        ...
        updateMetricCards(data) {
            ...
            $('.wcefp-metric[data-metric="bookings"] .wcefp-metric-value').text(
                this.formatNumber(data.total_bookings)
            );
            
            $('.wcefp-metric[data-metric="revenue"] .wcefp-metric-value').text(
            ...
            $('.wcefp-metric[data-metric="customers"] .wcefp-metric-value').text(
                this.formatNumber(data.total_customers)
            );
            
            // Update growth indicators
            this.updateGrowthIndicator('bookings', data.growth_rates.bookings);
            this.updateGrowthIndicator('revenue', data.growth_rates.revenue);
        }

        updateGrowthIndicator(metric, growthRate) {
            const indicator = $(`.wcefp-metric[data-metric="${metric}"] .wcefp-growth-indicator`);
            ...
            this.charts.bookingTrends = new Chart(ctx, {
                ...
                data: {
                    ...
                    datasets: [{
                        ...
                        backgroundColor: wcefp_analytics.chart_colors.primary + '20',
                        tension: 0.4,
                        fill: true
                    }, {
                        ...
                        backgroundColor: wcefp_analytics.chart_colors.secondary + '20',
                        tension: 0.4,
                        yAxisID: 'y1'
                    }]
                },
        ...
        renderRevenueByEventChart(data) {
            const ctx = document.getElementById('wcefp-revenue-by-event-chart');
        ...
        loadEventPerformance() {
            ...
            $.post(wcefp_analytics.ajax_url, data)
                ...
                .fail(() => this.showError('Failed to load event performance data'));
        }
            ...
            data.forEach(event => {
                const row = `
                    <tr>
                        ...
                        <td>${this.formatCurrency(event.avg_booking_value)}</td>
                        <td>
                            <div class="wcefp-occupancy-bar">
                                <div class="wcefp-occupancy-fill" style="width: ${event.occupancy_rate}%"></div>
                                <span class="wcefp-occupancy-text">${event.occupancy_rate.toFixed(1)}%</span>
                            </div>
                        </td>
                    </tr>
                `;
        ...
        updateRecentActivities(activities) {
            ...
            if (!activities || !activities.length) {
                container.append('<p>' + wcefp_analytics.strings.no_data + '</p>');
                return;
            }
                ...
                const listItem = `
                    <li class="wcefp-activity-item">
                        <div class="wcefp-activity-content">
                            <strong>${this.escapeHtml(activity.booking_title)}</strong>
                            <span class="wcefp-activity-email">${this.escapeHtml(activity.customer_email)}</span>
                        </div>
                        <div class="wcefp-activity-meta">
                            <span class="wcefp-activity-date">${this.formatDate(activity.created_date)}</span>
                            <span class="wcefp-activity-status status-${activity.status}">${activity.status}</span>
                        </div>
                    </li>
                `;
        ...
        handleExport(e) {
            ...
            window.open(`${wcefp_analytics.ajax_url}?${params.toString()}`, '_blank');
        }
        ...
        formatCurrency(amount) {
            return wcefp_analytics.currency_symbol + this.formatNumber(parseFloat(amount).toFixed(2));
        }
