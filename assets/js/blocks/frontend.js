...
( function ( $ ) {
	...
	const WCEFPBlocks = {
		...
		enhanceBookingForm( $block ) {
			...
			$block
				.find( '.wcefp-booking-form' )
				.addClass( 'wcefp-enhanced-form' );
			...
			$block
				...
				.on( 'focus', function () {
					$( this )
						.closest( '.wcefp-form-field' )
						.addClass( 'wcefp-field-focused' );
				} )
				.on( 'blur', function () {
					$( this )
						.closest( '.wcefp-form-field' )
						.removeClass( 'wcefp-field-focused' );
				} );
		},
		...
		enhanceEventList( $block ) {
			...
			$block.find( '.wcefp-btn' ).on( 'click', function ( e ) {
				...
				if ( $btn.data( 'ajax-action' ) ) {
					...
					setTimeout( () => {
						$btn.removeClass( 'wcefp-loading' ).prop(
							'disabled',
							false
						);
					}, 1000 );
