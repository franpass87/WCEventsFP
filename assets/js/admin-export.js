...
(function($) {
    ...
    const WCEFPExport = {
        ...
        bindEvents() {
            $('#wcefp-export-bookings-form').on('submit', this.handleBookingsExport);
            $('#wcefp-export-calendar-form').on('submit', this.handleCalendarExport);
        ...
        handleBookingsExport(e) {
            ...
            $button.prop('disabled', true).html(
                '<span class="dashicons dashicons-update-alt spin"></span> ' + wcefpExport.strings.exporting
            );
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        WCEFPExport.downloadFile(response.data.filename, response.data.content, 'text/csv');
                        WCEFPModals.showSuccess(
                            wcefpExport.strings.success + '\n' + 
                            sprintf('Downloaded %d bookings.', response.data.count)
                        );
                    } else {
                        WCEFPModals.showError(response.data.message || wcefpExport.strings.error);
                    }
                },
                error: function() {
        ...
        handleCalendarExport(e) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        WCEFPExport.downloadFile(response.data.filename, response.data.content, 'text/calendar');
                        WCEFPModals.showSuccess(
                            wcefpExport.strings.success + '\n' + 
                            sprintf('Downloaded calendar with %d events.', response.data.count)
                        );
                    } else {
                        WCEFPModals.showError(response.data.message || wcefpExport.strings.error);
                    }
