...
(function($) {
    ...
    const WCEFPSettings = {
        ...
        bindEvents: function() {
            // Tab navigation
            $(document).on('click', '.wcefp-nav-tab-wrapper .nav-tab', this.handleTabClick.bind(this));
            
            // Form submission
            $(document).on('submit', '.wcefp-settings-form', this.handleFormSubmit.bind(this));
            
            // Keyboard navigation for tabs
            $(document).on('keydown', '.wcefp-nav-tab-wrapper .nav-tab', this.handleTabKeydown.bind(this));
            
            // Help tips
            $(document).on('mouseenter', '.wcefp-help-tip', this.showHelpTip.bind(this));
            $(document).on('mouseleave', '.wcefp-help-tip', this.hideHelpTip.bind(this));
        },
         ...
         */
        switchTab: function(tabKey, updateUrl) {
            const $tabs = $('.wcefp-nav-tab-wrapper .nav-tab');
        ...
        initFormValidation: function() {
            ...
            $(document).on('input', '#wcefp_price_rules', this.validatePriceRules.bind(this));
            
            // Email validation
            $(document).on('blur', 'input[type="email"]', this.validateEmail.bind(this));
            
            // Number validation
            $(document).on('input', 'input[type="number"]', this.validateNumber.bind(this));
        },

        /**
         * Validate price rules JSON
         */
        validatePriceRules: function(e) {
        ...
        showFieldError: function($field, message) {
            ...
            const $error = $('<div class="wcefp-field-error" style="color: #d63638; font-size: 12px; margin-top: 5px;">')
                .text(message);
            
            $field.addClass('wcefp-field-invalid').css('border-color', '#d63638');
            $field.after($error);
        },
        ...
        showFieldSuccess: function($field, message) {
            ...
            const $success = $('<div class="wcefp-field-success" style="color: #00a32a; font-size: 12px; margin-top: 5px;">')
                .text(message);
            
            $field.removeClass('wcefp-field-invalid').css('border-color', '');
            $field.after($success);
        ...
        clearFieldMessages: function($field) {
            $field.nextAll('.wcefp-field-error, .wcefp-field-success').remove();
        },
        ...
        handleFormSubmit: function(e) {
            ...
            if ($form.find('.wcefp-field-invalid').length > 0) {
                e.preventDefault();
                this.showNotice('Correggi gli errori nel modulo prima di salvare.', 'error');
                return false;
            }
        ...
        showNotice: function(message, type) {
            ...
            const $notice = $('<div class="notice wcefp-notice notice-' + type + ' is-dismissible">')
                .append('<p>' + message + '</p>')
        ...
        initTooltips: function() {
            ...
            if ($('#wcefp-tooltip').length === 0) {
                $('body').append('<div id="wcefp-tooltip" class="wcefp-tooltip"></div>');
            }

            const $tooltip = $('#wcefp-tooltip');

            $(document).on('mouseenter', '[data-wcefp-tooltip]', function(e) {
            ...
            $(document).on('mouseleave', '[data-wcefp-tooltip]', function() {
                $tooltip.hide();
            });
        }
    };
