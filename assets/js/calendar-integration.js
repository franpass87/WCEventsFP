...
( function ( $ ) {
	...
	const CalendarIntegration = {
		...
		bindEvents() {
			...
			$( document ).on(
				'wcefp:booking_confirmed',
				this.addCalendarButtons
			);
			$( document ).on(
				'wcefp:content_updated',
				this.enhanceCalendarButtons
			);
		},
		...
		handleICSDownload( e, $button ) {
			...
			if ( navigator.msSaveBlob || window.downloadFile ) {
				...
				$.ajax( {
					...
					dataType: 'text',
					success( data ) {
						CalendarIntegration.downloadICSFile(
							data,
							'event.ics'
						);
					},
		 ...
		 * @param  $button
		 */
		getCalendarType( $button ) {
			if ( $button.hasClass( 'wcefp-google-calendar' ) ) return 'google';
			if ( $button.hasClass( 'wcefp-outlook-calendar' ) )
				return 'outlook';
		 ...
		 * @param  calendarType
		 */
		trackCalendarUsage( calendarType ) {
			// Send usage analytics
			if (
				typeof wcefpAnalytics !== 'undefined' &&
				wcefpAnalytics.enabled
			) {
				$.ajax( {
		 ...
		 * @param  bookingData
		 */
		addCalendarButtons( e, bookingData ) {
			...
			$.ajax( {
				...
				success( response ) {
					if ( response.success && response.data.buttons ) {
						...
						const $confirmation = $(
							'.wcefp-booking-confirmation, .wcefp-booking-success'
						);
		...
		generateCalendarUrls( eventData ) {
			const startDate = this.formatCalendarDate(
				eventData.date,
				eventData.time
			);
			const endDate = this.formatCalendarDate(
				eventData.date,
				eventData.time,
				2
			); // +2 hours
		 ...
		 * @param  hoursToAdd
		 */
		formatCalendarDate( date, time, hoursToAdd ) {
		...
		showSimpleModal( content ) {
			...
			// Close modal events
			$modal
				.find( '.wcefp-modal-close, .wcefp-modal-overlay' )
				.on( 'click', function () {
