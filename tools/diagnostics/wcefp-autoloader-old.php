<?php
/**
 * WCEventsFP Diagnostic Autoloader Wrapper
 * 
 ...
 * source of truth for class loading (T-02 consolidation).
 * 
 * @package WCEventsFP
...
/**
 * Diagnostic Autoloader - now a wrapper around the canonical autoloader
 * 
 * Provides enhanced diagnostics and error reporting while using the
 * canonical autoloader as the single source of truth.
 */
class WCEFP_Diagnostic_Autoloader {
    ...
    public function __construct($base_dir = null) {
        ...
        // Load the canonical autoloader first
        $this->ensure_canonical_autoloader();
        
        if ($this->debug) {
            error_log('WCEFP Diagnostic Autoloader: Initialized as wrapper (T-02)');
        }
    }
    
    /**
     * Ensure the canonical autoloader is loaded
     */
    private function ensure_canonical_autoloader() {
        if (!defined('WCEFP_CANONICAL_AUTOLOADER_LOADED')) {
            ...
            if (file_exists($canonical_path)) {
                ...
                if ($this->debug) {
                    error_log('WCEFP Diagnostic Autoloader: Loaded canonical autoloader');
                }
            } else {
                if ($this->debug) {
                    error_log('WCEFP Diagnostic Autoloader: ERROR - Cannot find canonical autoloader at ' . $canonical_path);
                }
            }
        }
    }
    ...
    /**
     * Register diagnostic wrapper (T-02: No longer registers duplicate)
     */
    public function register() {
        ...
        if (class_exists('WCEFP_Autoloader')) {
            ...
            if ($this->debug) {
                error_log('WCEFP Diagnostic Autoloader: Enhanced canonical autoloader with debug mode');
            }
        } else {
            if ($this->debug) {
                error_log('WCEFP Diagnostic Autoloader: WARNING - Canonical autoloader not available');
            }
        }
    }
    
    /**
     * Unregister (now a no-op since we don't register)
     */
    public function unregister() {
        // T-02: Nothing to unregister, just disable debug if possible
        if (class_exists('WCEFP_Autoloader')) {
